...
class SSD1306:
  ...
  def text_num(self, string, x, y, col=1):	# 在(x, y)处显示字符串，注意text()函数内置的字体是8x16的，暂时不能替换
    ...
    for s in string:
      ...
      start = y1+x1
      for num in range(8):
         self.buffer[start+num] = (self.buffer[start+num]<<(8-y0)>>(8-y0)) | (self.num_list[index][num]<<y0)
         # buffer低y0位  |  num_list低8-y0位
      
      start = y1+128+x1
      for num in range(8):
         self.buffer[start+num] = (self.num_list[index][num]>>(8-y0)) | (self.num_list[index][num+8]<<y0)
      ...
      if start>=8*128:
         continue
      for num in range(8):
         self.buffer[start+num] = (self.num_list[index][num+8]>>(8-y0)) | (self.buffer[start+num]<<y0)
  ...
  def img_16x16(self, img, x, y, col=1):	# 在(x, y)处显示字符串，注意text()函数内置的字体是8x16的，暂时不能替换
    ...
    start = y1+x1
    for num in range(16):
       self.buffer[start+num] = (self.buffer[start+num]<<(8-y0)>>(8-y0)) | (img[num]<<y0)
       # buffer低y0位  |  num_list低8-y0位
    start = y1+128+x1
    for num in range(16):
    ...
    if start>=8*128:
       return
    for num in range(16):
       self.buffer[start+num] = (img[num+16]>>(8-y0)) | (self.buffer[start+num]<<y0)
